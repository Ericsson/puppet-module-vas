...
describe 'vas' do
  ...
  on_supported_os(test_on).each do |_os, os_facts|
    describe 'parameter testing' do
      ...
      [true, false].each do |value|
        describe "with users_allow_hiera_merge set to valid #{value}" do
          let(:facts) do
            os_facts.merge(
              lsbmajdistrelease: os_facts[:os]['release']['major'], # Satisfy nisclient
              fqdn: 'hieramerge.example.local',
              parameter_tests: 'users_allow_hiera_merge',
            )
          end

          let(:params) do
            required_params.merge(
              users_allow_hiera_merge: value,
            )
          end

          result = if value == true
                     "fqdn@test.ing:242:242::::/bin/bash\nparam@test.ing:3:3::::/bin/bash\n"
                   else
                     "fqdn@test.ing:242:242::::/bin/bash\n"
                   end

          it { is_expected.to contain_file('vas_users_allow').with_content(header + result) }
        end
      end

      [true, false].each do |value|
        describe "with users_deny_hiera_merge set to valid #{value}" do
          let(:facts) do
            os_facts.merge(
              lsbmajdistrelease: os_facts[:os]['release']['major'], # Satisfy nisclient
              fqdn: 'hieramerge.example.local',
              parameter_tests: 'users_deny_hiera_merge',
            )
          end

          let(:params) do
            required_params.merge(
              users_deny_hiera_merge: value,
            )
          end
          ...
          it { is_expected.to contain_file('vas_users_deny').with_content(header + result) }
        end
      end

      [true, false].each do |value|
        describe "with user_override_hiera_merge set to valid #{value}" do
          let(:facts) do
            os_facts.merge(
              lsbmajdistrelease: os_facts[:os]['release']['major'], # Satisfy nisclient
              fqdn: 'hieramerge.example.local',
              parameter_tests: 'user_override_hiera_merge',
            )
          end

          let(:params) do
            required_params.merge(
              user_override_hiera_merge: value,
            )
          end
          ...
          it { is_expected.to contain_file('vas_user_override').with_content(header + result) }
        end
      end

      [true, false].each do |value|
        describe "with group_override_hiera_merge set to valid #{value}" do
          let(:facts) do
            os_facts.merge(
              lsbmajdistrelease: os_facts[:os]['release']['major'], # Satisfy nisclient
              fqdn: 'hieramerge.example.local',
              parameter_tests: 'group_override_hiera_merge',
            )
          end

          let(:params) do
            required_params.merge(
              group_override_hiera_merge: value,
            )
          end

          result = if value == true
                     "TEST\\ing:::localuser,fqdn@test.ing\nTEST\\ing:::localuser,param@test.ing\n"
                   else
                     "TEST\\ing:::localuser,fqdn@test.ing\n"
                   end

          it { is_expected.to contain_file('vas_group_override').with_content(header + result) }
        end
      end
