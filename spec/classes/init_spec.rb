...
describe 'vas' do
  ...
  describe 'packages' do
    context 'defaults on osfamily RedHat with lsbmajdistrelease 6' do
      it { is_expected.to contain_package('vasclnt').with('ensure' => 'installed') }
      it { is_expected.to contain_package('vasyp').with('ensure' => 'installed') }
      it { is_expected.to contain_package('vasgp').with('ensure' => 'installed') }
    end
  ...
  describe 'config' do
    ...
    context 'with vas_conf_vasypd_update_interval set' do
      ...
      it do
        is_expected.to contain_file('vas_config').with_content(%r{update-interval = 234577})
      end
    end
    ...
    context 'with unjoin_vas set to true and vas_domain fact set' do
      ...
      it { is_expected.to contain_class('vas') }
      it { is_expected.to contain_exec('vas_unjoin') }
    ...
    context 'with non-UPM configuration' do
      ...
      it { is_expected.to contain_file('vas_config').with_content(%r{^\s*user-search-path = OU=Users,DC=example,DC=com$}) }
      it { is_expected.to contain_file('vas_config').with_content(%r{^\s*group-search-path = OU=Groups,DC=example,DC=com$}) }
      it { is_expected.to contain_file('vas_config').with_content(%r{^\s*root-update-mode = force-if-needed$}) }
      it { is_expected.to contain_file('vas_config').with_content(%r{^\s*group-update-mode = force-if-needed$}) }

      it { is_expected.to contain_class('pam') }
      it { is_expected.to contain_exec('vasinst').that_comes_before('Class[pam]') }
      it { is_expected.to contain_exec('vasinst').with_command(%r{-u OU=Users,DC=example,DC=com}) }
      it { is_expected.to contain_exec('vasinst').with_command(%r{-g OU=Groups,DC=example,DC=com}) }
      ...
      it { is_expected.not_to contain_file('vas_config').with_content(%r{^\s*upm-search-path}) }
      it { is_expected.not_to contain_exec('vasinst').with_command(%r{-p OU=UPM,DC=example,DC=com}) }
    end
  end
  context 'new UPM configuration' do
    ...
    it { is_expected.to contain_exec('vasinst').with_command(%r{-c OU=Computers,DC=example,DC=com}) }
    it { is_expected.to contain_exec('vasinst').with_command(%r{-n host.example.com}) }
    it { is_expected.not_to contain_exec('vasinst').with_command(%r{-g}) }
  end
  ...
  context 'with join_domain_controllers unset' do
    it { is_expected.to contain_exec('vasinst').with_command(%r{#{jdc_before}  #{jdc_after}}) }
  end

  context 'with join_domain_controllers as a string' do
    let(:params) { { join_domain_controllers: 'dc01.example.com' } }

    it { is_expected.to contain_exec('vasinst').with_command(%r{#{jdc_before} dc01.example.com #{jdc_after}}) }
  end

  context 'with join_domain_controllers as an array' do
    let(:params) { { join_domain_controllers: ['dc01.example.com', 'dc02.example.com'] } }

    it { is_expected.to contain_exec('vasinst').with_command(%r{#{jdc_before} dc01.example.com dc02.example.com #{jdc_after}}) }
  end
    ...
    describe 'hiera merge parameters' do
      ...
      [true, false, 'true', 'false'].each do |value|
        context "when #{parameter} is set to #{value} (as #{value.class})" do
          ...
          it { is_expected.to contain_file(v[:filename].to_s).with_content(%r{^#{content}}) }
        end
      end
    end
  end
  ...
  describe 'other' do
    context 'fail on unsupported kernel' do
      ...
      it 'fails' do
        expect { is_expected.to contain_class('vas') }.to raise_error(Puppet::Error, %r{Vas module support Linux and SunOS kernels\.})
      end
    end
  ...
  describe 'with symlink_vastool_binary' do
    ['true', true].each do |value|
      context "set to #{value} (default)" do
        ...
        it do
          is_expected.to contain_file('vastool_symlink').with(
    ...
    context 'enabled with invalid vastool_binary' do
      ...
      it 'fails' do
        expect { is_expected.to contain_class('vas') }.to raise_error(Puppet::Error, %r{is not an absolute path})
      end
    end
  ...
  context 'with kdcs set to valid array [test1.domain.local test2.domain.local]' do
    ...
    it { is_expected.to contain_file('vas_config').with_content(%r{^  kdc = kdc1.domain.local:88 kdc2.domain.local:88\n  kpasswd_server = kdc1.domain.local:464 kdc2.domain.local:464}) }
  end
  ...
  context 'with kdc_port set to valid integer 242 when kdcs is set to a valid array' do
    ...
    it { is_expected.to contain_file('vas_config').with_content(%r{^  kdc = kdc1.domain.local:242 kdc2.domain.local:242}) }
  end
  ...
  context 'with kpasswd_server_port set to valid integer 242 when kdcs is set to a valid array' do
    ...
    it { is_expected.to contain_file('vas_config').with_content(%r{^  kpasswd_server = kdc1.domain.local:242 kdc2.domain.local:242}) }
  end
  ...
  describe 'variable data type and content validations' do
    ...
    validations = {
      ...
      'boolean' => {
        name: ['user_override_hiera_merge', 'group_override_hiera_merge', 'domain_change', 'unjoin_vas', 'vas_conf_vas_auth_allow_disconnected_auth', 'vas_conf_vas_auth_expand_ac_groups',
               'use_srv_infocache'],
        ...
        var[:validf].each do |validf|
          context "when #{var_name} (#{type}) is set to valid (but failing) #{validf} (as #{validf.class})" do
            ...
            it 'fails' do
              ...
              } .to raise_error(Puppet::Error, %r{(Error while evaluating a Function Call|getaddrinfo: Name or service not known|Connection refused - connect)})
            end
          end
        end
